(() => {
  ...
  function replaceProhibitedWord(root) {
    ...
    nodes.forEach((node) => {
      if (node.nodeValue?.includes('병목')) {
        node.nodeValue = node.nodeValue.replaceAll('병목', '우선성장 지점');
      }
    });
  }
  ...
  function cleanCopy() {
    ...
    if (synthesisType?.textContent.includes('집중 병목형')) {
      synthesisType.textContent = '집중 조율형';
    }
