...
assert(validDownstreamCounts.has(producedByTheme.recharge.length), "recharge assets are in a partial or invalid state");
...
assert(producedByTheme.future.length !== 15 || producedByTheme.recharge.length === 15, "future deck completed before recharge deck");
assert(producedByTheme.kind.length !== 15 || (producedByTheme.recharge.length === 15 && producedByTheme.future.length === 15), "kind deck completed before earlier A04 batches");
...
const report = {
  ...
  downstreamAssetCounts: Object.fromEntries(Object.entries(producedByTheme).map(([theme, ids]) => [theme, ids.length])),
  batch1GateSnapshot: "PASS",
  errors,
};
