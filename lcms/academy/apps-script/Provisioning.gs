...
function provisionStudentRow_(ss, rowNumber, options) {
  ...
  try {
    ...
    if (!student.id || !student.studentName || !isValidEmail_(student.email)) throw new Error('수강생 이름 또는 Google 계정 이메일이 올바르지 않습니다.');
    ...
    grantDriveAccess_(ss, course, student.email);
    ...
    invalidateStudentSessions_(ss, student.id, '코드 재발급');
...
function grantDriveAccess_(ss, course, email) {
  if (!course.driveFolderId) throw new Error('과정설정 시트의 Drive폴더ID가 비어 있습니다.');
  const errors = [];
  try {
    DriveApp.getFolderById(course.driveFolderId).addViewer(email);
  } catch (error) {
    errors.push(`폴더 권한: ${error.message}`);
  }

  getDriveMediaFileIds_(ss, course.courseId).forEach(function(fileId) {
    try { DriveApp.getFileById(fileId).addViewer(email); }
    catch (error) { errors.push(`${fileId}: ${error.message}`); }
  });

  if (errors.length) throw new Error('Drive 열람권한 부여 실패 · ' + errors.join(' / '));
}

function revokeDriveAccess_(ss, course, email) {
  if (!email) return;
  try { if (course.driveFolderId) DriveApp.getFolderById(course.driveFolderId).revokePermissions(email); }
  catch (error) { console.warn('Folder revoke failed:', error); }
  getDriveMediaFileIds_(ss, course.courseId).forEach(function(fileId) {
    try { DriveApp.getFileById(fileId).revokePermissions(email); }
    catch (error) { console.warn('File revoke failed:', fileId, error); }
  });
}

function getDriveMediaFileIds_(ss, courseId) {
  const sheet = ss.getSheetByName(RSEDU_ACADEMY.SHEETS.MEDIA);
  const lastRow = sheet.getLastRow();
  if (lastRow < RSEDU_ACADEMY.DATA_START_ROW) return [];
  return sheet.getRange(RSEDU_ACADEMY.DATA_START_ROW, 1, lastRow - RSEDU_ACADEMY.HEADER_ROW, 9).getValues()
    .filter(function(row) {
      return String(row[0]) === courseId && String(row[3]).toUpperCase() === 'DRIVE' && String(row[4]).trim() && String(row[7]).trim() !== '중지';
    })
    .map(function(row) { return String(row[4]).trim(); })
...
function sendAccessEmail_(course, student, code, expiresAt) {
  ...
  const plainBody = [
    ...
    `수강기간: ${expiry}까지`,
    '',
    `Drive 영상은 신청서에 입력한 Google 계정(${student.email})으로 로그인해야 재생됩니다.`,
    '입장코드와 강의 링크를 타인에게 공유하지 마세요.'
  ].join('\n');
  ...
  const htmlBody = `
    <div style="max-width:640px;margin:0 auto;padding:32px;font-family:Arial,'Noto Sans KR',sans-serif;color:#071c43;background:#f5f3ed">
      <div style="padding:34px;background:#071c43;color:#fff">
        ...
        <p style="margin:0;color:rgba(255,255,255,.72);line-height:1.7">결제 확인과 Google Drive 열람권한 등록이 완료되었습니다.</p>
      </div>
      <div style="padding:32px;background:#fff">
        ...
        <table style="width:100%;margin-top:24px;border-collapse:collapse;font-size:13px">
          <tr><td style="padding:10px 0;color:#68738a">등록 Google 계정</td><td style="padding:10px 0;text-align:right;font-weight:800">${escapeHtml_(student.email)}</td></tr>
        ...
        <p style="margin:24px 0 0;padding:16px;background:#f7f7f4;color:#58647a;font-size:12px;line-height:1.7">Drive 영상이 재생되지 않으면 브라우저에서 위 Google 계정으로 로그인했는지 확인한 뒤, 영상 우측 아래의 ‘Drive에서 열기’를 이용해 주세요. 입장코드와 강의 링크의 타인 공유는 허용되지 않습니다.</p>
      </div>
    </div>`;
  ...
  if (course.replyEmail && isValidEmail_(course.replyEmail)) options.replyTo = course.replyEmail;
