...
async function authorize(request, env, origin) {
  ...
  const part = Number(parsedBody.value.part);
  const media = mediaCatalogEntry(week, part);
  ...
  if (media.status !== 'published') {
    return json(
      { ok: false, message: '이 영상은 아직 공개 준비 중입니다.' },
      403,
      origin,
      { 'Cache-Control': 'no-store' }
    );
  ...
  const signature = await sign(
    env.PLAYBACK_SECRET,
    signaturePayload(courseId, week, part, media.mediaId, objectKey, expiresAt)
  );
  const playbackUrl = new URL(request.url);
  playbackUrl.pathname = `/media/${encodeURIComponent(courseId)}/${week}/${part}`;
  playbackUrl.search = '';
...
async function serveMedia(request, env, origin, url) {
  ...
  const match = /^\/media\/([^/]+)\/(\d+)\/(\d+)$/.exec(url.pathname);
...
function mediaCatalogEntry(week, part) {
  const media = MEDIA_CATALOG.get(`${Number(week)}:${Number(part)}`);
  if (!media || media.courseId !== COURSE_ID || media.week !== Number(week) || media.part !== Number(part)) return null;
  return media;
}

function signaturePayload(courseId, week, part, mediaId, objectKey, expiresAt) {
  return `${courseId}|${week}|${part}|${mediaId}|${objectKey}|${expiresAt}`;
}
