...
test('/authorize rejects week 12 before calling Apps Script', async () => {
  let validationCalls = 0;
  globalThis.fetch = async () => {
    validationCalls += 1;
    throw new Error('must not be called');
...
test('/authorize uses workerValidate POST contract and caps playback at session expiry', async () => {
  ...
  assert.equal(playbackUrl.pathname, `/media/${COURSE_ID}/1`);
...
test('signed /media Range request reads only the fixed R2 key and returns 206', async () => {
  ...
  const playbackUrl = await issuePlaybackUrl(env, 1);
  ...
  assert.equal(getKey, 'lmc/week-01.mp4');
...
test('/media rejects disallowed origins and tampered signatures before R2 access', async () => {
  ...
  const playbackUrl = await issuePlaybackUrl(env, 2);
...
test('/media rejects week 12 and invalid or multi-part Range requests', async () => {
  ...
  weekTwelveUrl.pathname = `/media/${COURSE_ID}/12`;
...
test('HEAD /media returns metadata without reading the object body', async () => {
  ...
  const playbackUrl = await issuePlaybackUrl(env, 11);
  ...
  assert.equal(response.status, 200);
  assert.equal(headKey, 'lmc/week-11.mp4');
...
test('an already signed /media URL keeps streaming during an Apps Script outage', async () => {
  const env = baseEnv();
  const playbackUrl = await issuePlaybackUrl(env, 3);
...
async function issuePlaybackUrl(env, week) {
  globalThis.fetch = async () => Response.json({
    ok: true,
    valid: true,
    studentId: 'STU_TEST',
  ...
  const response = await worker.fetch(authorizeRequest({ week }), env);
  assert.equal(response.status, 200);
