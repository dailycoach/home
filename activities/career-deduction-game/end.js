...
function openFeedback(type, title, message, buttonLabel, callback) {
  ...
  $('#feedbackKicker').textContent = type === 'correct' ? 'NICE! JOB FOUND' : 'SO CLOSE! TRY AGAIN';
...
function finalTeamHtml(side) {
  return `
    <article class="final-team ${side.toLowerCase()}">
      <div class="final-team-head">
        <span>${side === 'A' ? '🔵' : '🟠'}</span>
        <div><small>TEAM ${side} JOB CARDS</small><h2>${escapeHtml(teamLabel(side))}</h2></div>
      </div>
      <div class="final-card-grid">
        ${state.rosters[side].map(entry => `
          <div class="final-card">
            ...
            <span>${CATEGORIES[entry.job.cat].name}</span>
          </div>
        `).join('')}
      </div>
    </article>
...
function renderGameEnd() {
  ...
  $('#gameEndTitle').textContent = `${teamLabel(winner)} 승리!`;
  $('#gameEndCopy').textContent = `${teamLabel(winner)}이(가) 상대 팀 ${state.teamSizes[opponent(winner)]}명의 직업을 모두 먼저 밝혔습니다.`;
  $('#finalReveal').innerHTML = finalTeamHtml('A') + finalTeamHtml('B');
}
...
function resetAll() {
  clearTimers();
  if (confirm('현재 팀 배틀을 종료하고 처음 화면으로 돌아갈까요?')) {
...
$('#hintTarget').addEventListener('change', renderHintChoices);
$('#hintChoices').addEventListener('click', event => {
  const button = event.target.closest('[data-hint-type]');
  if (button) applyBonusHint(button.dataset.hintType);
});

$$('[data-close]').forEach(button => button.addEventListener('click', () => {
  const id = button.dataset.close;
  closeModal(id);
  if (state.phase === 'play' && (id === 'actionModal' || id === 'guessModal') && !state.timer) {
    startTurnTimer(false);
  }
}));

$$('[data-action]').forEach(button => button.addEventListener('click', () => {
  if (button.dataset.action === 'guess') openGuess();
  else actionPrompt(button.dataset.action);
}));

$$('[data-bonus-winner]').forEach(button => button.addEventListener('click', () => {
  awardBonus(button.dataset.bonusWinner);
}));

$('#resumeBtn').addEventListener('click', () => {
  if (!load()) return;
  $('#teamNameA').value = state.teamNames.A;
  $('#teamNameB').value = state.teamNames.B;
  renderTeamInputs('A');
  renderTeamInputs('B');
  const timerRadio = $(`#t${state.timerSeconds}`);
  const bonusRadio = $(`#b${state.bonusCount}`);
...
window.addEventListener('keydown', event => {
  if (event.key === 'Enter' && !$('#feedbackModal').classList.contains('hidden')) confirmFeedback();
});
