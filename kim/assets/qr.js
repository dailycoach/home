/* QRCode: copyright (c) 2009 Kazuhiko Arase, MIT. Node adaptation: qrcode-terminal 0.12.0, Apache-2.0. See THIRD_PARTY_NOTICES.txt. */
(function(global){
"use strict";
const modules = {
...
function require(id) {
  ...
  if (!modules[key]) throw new Error('Unknown bundled QR module: ' + key);
  const module = { exports: {} }; cache[key] = module;
  modules[key](module,module.exports,require); return module.exports;
}
const QRCode = require('index');
const levels = require('QRErrorCorrectLevel');
global.DailyQR = function(text) {
  const bytes = new TextEncoder().encode(String(text));
  if (bytes.length > 2200) throw new Error('QR payload too large');
  let binary = ''; for (const byte of bytes) binary += String.fromCharCode(byte);
  const qr = new QRCode(-1, levels.M); qr.addData(binary); qr.make(); return qr;
};
})(globalThis);
