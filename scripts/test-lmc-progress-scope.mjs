...
const legacy = {
  completed: { [courseId]: ['module-01'] },
  notes: { [`${courseId}:module-01`]: '기존 수강생 기록' },
  lastViewed: { [courseId]: 'module-01' },
  playback: { [`${courseId}:module-01`]: 35 }
};
...
assert.equal(store.load().notes[`${courseId}:module-01`], '기존 수강생 기록');
...
progressA.notes[`${courseId}:module-02`] = 'A 전용 메모';
store.save(progressA);

store.setAuthenticatedStudent(studentB);
assert.deepEqual(
  JSON.parse(JSON.stringify(store.load())),
  { completed: {}, notes: {}, lastViewed: {}, playback: {}, updatedAt: null },
  'a second student must not inherit the first student progress'
);
const progressB = store.load();
progressB.notes[`${courseId}:module-02`] = 'B 전용 메모';
store.save(progressB);

store.setAuthenticatedStudent(studentA);
assert.equal(store.load().notes[`${courseId}:module-02`], 'A 전용 메모');
assert.notEqual(store.load().notes[`${courseId}:module-02`], 'B 전용 메모');
...
localStorage.setItem('rsedu-academy-progress:v1', JSON.stringify({
  notes: { [`${courseId}:module-03`]: '뒤늦게 생긴 레거시 기록' }
}));
store.setAuthenticatedStudent(studentC);
assert.equal(store.load().notes[`${courseId}:module-03`], undefined, 'legacy data must never migrate to a second student');
