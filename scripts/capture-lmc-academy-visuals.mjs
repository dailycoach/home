...
const OUTPUT_DIR = path.resolve('artifacts/lmc-academy-v2-visual-audit');
...
const targets = [
  ...
  {
    name: 'lesson-week-01-part-01',
    url: `/lcms/academy/lesson.html?course=${COURSE_ID}&week=1&part=1`,
    waitFor: '#r2VideoPlayer',
    authenticated: true,
    expectVideo: true
  },
  {
    name: 'lesson-week-01-part-01-lifecycle',
    url: `/lcms/academy/lesson.html?course=${COURSE_ID}&week=1&part=1`,
    waitFor: '#r2VideoPlayer',
    authenticated: true,
    expectVideo: true,
  ...
  {
    name: 'lesson-week-01-part-01-pending',
    url: `/lcms/academy/lesson.html?course=${COURSE_ID}&week=1&part=1`,
  ...
  {
    name: 'lesson-week-01-part-01-error',
    url: `/lcms/academy/lesson.html?course=${COURSE_ID}&week=1&part=1`,
  ...
  {
    name: 'lesson-week-03-part-04',
    url: `/lcms/academy/lesson.html?course=${COURSE_ID}&week=3&part=4`,
    waitFor: '#r2VideoPlayer',
    authenticated: true,
    expectVideo: true
  },
  {
    name: 'lesson-week-08-part-08',
    url: `/lcms/academy/lesson.html?course=${COURSE_ID}&week=8&part=8`,
    waitFor: '#r2VideoPlayer',
    authenticated: true,
    expectVideo: true
  },
  {
    name: 'lesson-week-11-part-07',
    url: `/lcms/academy/lesson.html?course=${COURSE_ID}&week=11&part=7`,
    waitFor: '#r2VideoPlayer',
    authenticated: true,
    expectVideo: true
  },
  {
    name: 'lesson-week-12',
    url: `/lcms/academy/lesson.html?course=${COURSE_ID}&week=12`,
...
async function configureRoutes(context, stats, options = {}) {
  ...
  await context.route(`${WORKER_ORIGIN}/authorize`, async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ok: true,
        url: `${WORKER_ORIGIN}/media/${COURSE_ID}/${requestBody.week}/${requestBody.part}?exp=${expiresAtSeconds}&sig=visual-fixture`,
        expiresAt: new Date(expiresAtSeconds * 1000).toISOString(),
        week: requestBody.week,
        part: requestBody.part,
        mediaId: `lmc-w${String(requestBody.week).padStart(2, '0')}-p${String(requestBody.part).padStart(2, '0')}`
      })
    });
  });
  ...
  for (const target of targets) {
      ...
      if (sessionStorage.getItem(marker) !== '1') {
        ...
        if (exercisePlayback) {
          localStorage.setItem(progressKey, JSON.stringify({
            ...
            playback: { [`${courseId}:week-01-part-01`]: 15 },
            finalWeeks: {},
    ...
    if (target.exercisePlayback) {
      playbackLifecycle = await page.evaluate(({ courseId, progressKey }) => {
        ...
        const savedBeforeRefresh = JSON.parse(localStorage.getItem(progressKey) || '{}')
          .playback?.[`${courseId}:week-01-part-01`];
      ...
      const afterRefresh = await page.evaluate(({ courseId, progressKey }) => {
        ...
        return {
          ...
          playbackCleared: !Object.prototype.hasOwnProperty.call(
            progress.playback || {},
            `${courseId}:week-01-part-01`
          ),
          autoCompleted: Array.isArray(progress.completed?.[courseId])
            && progress.completed[courseId].includes('week-01-part-01'),
          completeButton: Boolean(document.querySelector('.lesson-complete-button.is-complete'))
        };
      }, { courseId: COURSE_ID, progressKey: PROGRESS_KEY });
    ...
    const metrics = await page.evaluate(({
      ...
      return {
        ...
        hasPublishedMediaCopy: /R2 비공개 스트리밍/.test(bodyText),
